 ...
 */
export const buttonVariants = cva("inline-flex items-center gap-2 rounded-md font-medium disabled:pointer-events-none disabled:opacity-50", {
    variants: {
        variant: {
...
export function Button({ className, variant, size, type, ...props }) {
    return (_jsx("button", { 
        ...
        type: type ?? "button", className: cn(buttonVariants({ variant, size }), className), ...props }));
}
