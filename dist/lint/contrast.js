...
export function parseOklch(value) {
    ...
    if (!match)
        throw new Error(`"${value}" is not an oklch(l c h) colour`);
...
const asOklch = (colour) => typeof colour === "string" ? parseOklch(colour) : colour;
...
function topLevelBlocks(css) {
    ...
    for (let i = 0; i < css.length; i++) {
        if (css[i] === "{") {
            ...
            if (depth === 0)
                throw new Error(`unbalanced "}" at index ${i} of the voice CSS`);
            if (--depth === 0) {
                blocks.push({
    ...
    if (depth !== 0)
        throw new Error(`unterminated block at index ${openAt} of the voice CSS`);
    return blocks;
}
/**
...
export function tokenBlocks(css, includeAlpha = false) {
    ...
    for (const block of declaring) {
        if (block.prelude !== LIGHT && block.prelude !== DARK)
            throw new Error(`the voice CSS declares oklch tokens on "${block.prelude}", but the floors only ` +
                `iterate "${LIGHT}" and "${DARK}" — a palette on any other selector goes unchecked`);
        if (blocks.has(block.prelude))
            throw new Error(`the voice CSS declares "${block.prelude}" twice, so one of the two would go unchecked`);
