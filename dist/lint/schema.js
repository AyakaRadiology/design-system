 ...
 */
export const VOICE_ROLES = new Set([
    "surface",
    "border",
    "text",
    "accent",
 ...
 */
export const SCALE_ROLES = new Set([
    "type",
    "font",
    "z",
    "motion",
...
export function loadPackageSchema() {
    return parseSchema(JSON.parse(readFileSync(schemaPath(), "utf8")));
}
function checkKnownAndTyped(declared, schema) {
    const violations = [];
...
function missingFrom(declared, schema, roles, where, predicate = () => true) {
    const violations = [];
    for (const [name, spec] of Object.entries(schema.tokens)) {
        if (!spec.required || !roles.has(spec.role) || !predicate(spec))
            continue;
 ...
 */
export function validateVoice(css, schema) {
    const declared = collectCustomProperties(css);
    const violations = missingFrom(declared, schema, VOICE_ROLES, ROOT);
    const hasDark = [...declared.values()].some((occurrences) => occurrences.some((occurrence) => occurrence.selector === DARK));
    if (hasDark)
        violations.push(...missingFrom(declared, schema, VOICE_ROLES, DARK, (spec) => spec.type === "color"));
 ...
 * checked by nobody.
 */
export function validateScales(css, schema) {
    const declared = collectCustomProperties(css);
    const violations = [];
    for (const [name, spec] of Object.entries(schema.tokens)) {
        if (!spec.required || !SCALE_ROLES.has(spec.role))
            continue;
        if (!declared.has(name))
            violations.push({ code: "missing", token: name, where: ROOT });
    }
    violations.push(...checkKnownAndTyped(declared, schema));
    return violations;
}
 ...
 */
export function validateTailwindMapping(css, schema) {
    const declared = collectCustomProperties(css);
    const violations = [];
    for (const [name, spec] of Object.entries(schema.tokens)) {
        if (spec.type !== "color")
