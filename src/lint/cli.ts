...
export function parseArgs(argv: readonly string[]): Options | "help" {
    const options: Options = { config: DEFAULT_CONFIG, format: "text" };
    for (let i = 0; i < argv.length; i++) {
        const arg = argv[i];
        if (arg === "--help" || arg === "-h") return "help";
        if (arg === "--config") {
            const value = argv[++i];
            if (value === undefined) throw new ConfigError("--config needs a path");
...
export function main(
    ...
    try {
        ...
        writeError(`design-lint: ${(error as Error).message}`);
        writeError(USAGE);
        return 2;
    }
    if (options === "help") {
    ...
    if (options.format === "json") {
        write(
            JSON.stringify(
                { findings: result.findings, filesChecked: result.filesChecked },
                null,
                2,
            ),
        );
    ...
    if (result.findings.length > 0) write(formatText(result.findings));
    ...
    if (result.filesChecked === 0) {
        writeError(
            `design-lint: the "include" patterns matched no files under ${process.cwd()} — the gate would have checked nothing`,
        );
        return 2;
    }
    ...
    write(
        `design-lint: ${result.filesChecked} file(s), ${errors} error(s), ${warnings} warning(s)`,
    );
